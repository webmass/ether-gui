...
import ChakraUIStyledMethod from './StyledComponents/ChakraUIStyledMethod';
...
const themes = [
    { name: 'chakra', label: 'Chakra UI', component: ChakraUIStyledMethod, preferences: { wording: {}, styles: {} } },
...
const ExampleContractUI = ({ contract }) => {
    ...
    const themeLinks = themes.map((t, i) => {
        return <a
            key={i}
            href={`#${t.name}`}
            className={theme.name === t.name ? 'active-theme' : ''}
            data-testid={`theme-link-${theme.name}`}
            onClick={() => setTheme(t)}>{t.label}</a>
    })

    return (
        <div>
            <div className={titleClass}>Easily customize appearance, themed examples :</div>
            <div className="App-themes">
                {themeLinks}
            </div>

            <div className={titleClass}>"ExampleContract" methods :</div>
